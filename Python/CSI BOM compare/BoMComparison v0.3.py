...
#function to clear everything, path and result
    def clearAll(self):
        ...
        self.bomPath2.setText('')
        self.resultTable.clear()
#function to check toggle button status
    def changeResult(self):
        if self.refreshButton.isChecked():
            self.compareBoMall()
        else:
            self.compareBoMs()
#function to compare two BoMs show all parts
    def compareBoMall(self):
        if not self.bomPath1.text():
            QMessageBox.information(self,'Information','Please open the BoMs first!')
        else:
            #reset table
            self.resultTable.clear()
            ...
            nodesiMerge = pd.concat([df1Nodesi.rename({'Item':'Item_'+BoMNAME1,'Description':'Description_'+BoMNAME1},axis=1),df2Nodesi.rename({'Level':'Level2','Item':'Item_'+BoMNAME2,'Description':'Description_'+BoMNAME2,'Ref Designator':'Ref2'},axis=1)],axis=0).reset_index(drop=True)
...
#function to compare two BoMs but show difference only
    def compareBoMs(self):
        if not self.bomPath1.text():
            ...
            nodesiMergeUniq = pd.concat([nodesiuniq1.rename({'Item':'Item_'+BoMNAME1,'Description':'Description_'+BoMNAME1},axis=1),nodesiuniq2.rename({'Level':'Level2','Item':'Item_'+BoMNAME2,'Description':'Description_'+BoMNAME2,'Ref Designator':'Ref2'},axis=1)],axis=0).reset_index(drop=True)
