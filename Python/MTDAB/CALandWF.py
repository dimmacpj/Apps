...
def getInfo():
    ...
    acAngle = 15.010        # Input AC Phase Angle, unit is degree
...
def WFsPlot(inductanceL, switchPeriod, psPeriod, mcVMajor, mcVMinor, secVrep, targetIRatio, ipVamplitude, opVdc, ipVaInst, ipVbInst, ipVcInst, opPower):
    ...
    # Create plot 1 for Input AC Sine Waveforms
    ...
    # Create plot 2 for Primary, Secondary and Inductor voltage waveforms, and Inductor current waveform
    # Calculate data
    ...
    priVWF = [(0, 0), (0, mcVMajor), (tMajor*1e6, mcVMajor), (tMajor*1e6, mcVMinor), ((tMajor+tMinor)*1e6, mcVMinor), ((tMajor+tMinor)*1e6, 0), (switchPeriod/2*1e6, 0), (switchPeriod/2*1e6, -mcVMajor), 
    ...
    inductorVWF = [(0, 0-(-secVrep)), (0, mcVMajor-(-secVrep)), (psPeriod*1e6, mcVMajor-(-secVrep)), (psPeriod*1e6, mcVMajor-secVrep), (tMajor*1e6, mcVMajor-secVrep), (tMajor*1e6, mcVMinor-secVrep), ((tMajor+tMinor)*1e6, mcVMinor-secVrep), 
                   ((tMajor+tMinor)*1e6, 0-secVrep), (switchPeriod/2*1e6, 0-secVrep), (switchPeriod/2*1e6, -mcVMajor-secVrep), ((switchPeriod/2+psPeriod)*1e6, -mcVMajor-secVrep), ((switchPeriod/2+psPeriod)*1e6, -mcVMajor-(-secVrep)),
                   ((switchPeriod/2+tMajor)*1e6, -mcVMajor-(-secVrep)), ((switchPeriod/2+tMajor)*1e6, -mcVMinor-(-secVrep)), ((switchPeriod/2+tMajor+tMinor)*1e6, -mcVMinor-(-secVrep)), ((switchPeriod/2+tMajor+tMinor)*1e6, 0-(-secVrep)),
                   (switchPeriod*1e6, 0-(-secVrep))]

    line1, = axs2.plot(*zip(*secVWF), color = 'tab:blue')
    line2, = axs2.plot(*zip(*priVWF), color = 'tab:orange')
    line3, = axs2.plot(*zip(*inductorVWF), color = 'tab:red')
...
def MTDABcal(inductanceL, switchPeriod, psPeriod, mcVMajor, mcVMinor, secVrep, targetIRatio, ipVaInst, ipVbInst, ipVcInst, opPower):
    ...
    i1 = sp.simplify(i1.expand().subs([(Vmn, mcVMinor), (Vs, secVrep), (Ll, inductanceL), (Vmj, mcVMajor), (Tps, psPeriod)]))     
    print(i1)
    ...
    for i in range(len(periods)):
        if periods[i][0] > periods[i][1]:    
            actualImj = - iMJ.subs([(Vmn, mcVMinor), (Vmj, mcVMajor), (Vs, secVrep), (Tps, psPeriod), (Ll, inductanceL), (Ts, switchPeriod), (Tmj, periods[i][0]), (Tmn, periods[i][1])])
            actualImn = - iMN.subs([(Vmn, mcVMinor), (Vmj, mcVMajor), (Vs, secVrep), (Tps, psPeriod), (Ll, inductanceL), (Ts, switchPeriod), (Tmj, periods[i][0]), (Tmn, periods[i][1])])
    ...
    '''
    """ Function to calculate the Major current and the Minor current based on knowing the ratio of the two."""
    Vmj, Vmn, Vs, Ll, Tps, Tmj, Tmn, Ts = sp.symbols('Vmj, Vmn, Vs, Ll, Tps, iTmj, Tmn, Ts', positive = True)
